...
export function StockMovements({ canAdd = true, canModify = true }: { canAdd?: boolean; canModify?: boolean }) {
    ...
    const openCreate = () => { setForm({ ...blank(), warehouseID: warehouses[0]?.warehouseID || 0, productID: products[0]?.productID || 0 }); setError(""); setModal("create"); };
    const openDelete = (r: StockMovement) => { setSelected(r); setConfirmAction("delete"); };
    ...
    const handlePreSave = () => {
        ...
        if (!form.movementReference.trim()) return setError("Reference is required.");
        setConfirmAction("add");
    };

    const executeAction = () => {
        if (confirmAction === "add") addStockMovement({ ...form, processedBy: currentUserID });
        else if (confirmAction === "delete" && selected) deleteStockMovement(selected.stockMovementID);
    ...
    const columns = [
        { key: "stockMovementID", label: "ID", sortable: true, width: "100px", render: (r: StockMovement) => <MonoValue value={r.stockMovementID} /> },
        { key: "movementDate", label: "Date", sortable: true, render: (r: StockMovement) => <span className="text-xs text-gray-500">{new Date(r.movementDate).toLocaleString()}</span> },
        { key: "movementType", label: "Type", render: (r: StockMovement) => <Badge label={r.movementType} color={typeColors[r.movementType]} /> },
        { key: "productName", label: "Product", align: "left" as const, render: (r: StockMovement) => <span className="font-medium">{r.productName}</span> },
        { key: "warehouseName", label: "Warehouse", align: "left" as const, render: (r: StockMovement) => <span className="text-sm text-gray-500">{r.warehouseName}</span> },
        {
            key: "movementQuantity", label: "Qty", render: (r: StockMovement) => {
                const isPos = ["Stock In", "Return"].includes(r.movementType) || (r.movementType === "Adjustment" && r.movementQuantity > 0);
                return <span className={`font-mono text-sm font-semibold ${isPos ? "text-emerald-500" : "text-red-500"}`}>{isPos ? "+" : ""}{r.movementQuantity}</span>;
            }
        },
        { key: "movementReference", label: "Ref", render: (r: StockMovement) => <MonoValue value={r.movementReference} /> },
        ...(canModify ? [{
            key: "actions", label: "Actions", width: "50px", render: (r: StockMovement) => (
    ...
    return (
        <div className="space-y-4">
            ...
            <DataTable title="History" subtitle={`${stockMovements.length} records`} columns={columns} data={stockMovements} searchFields={["movementReference", "productName"]} rowKey={(r) => r.stockMovementID}
            ...
            <Modal title="Record Stock Movement" open={modal === "create"} onClose={() => setModal(null)} onSubmit={handlePreSave} submitLabel="Record Movement" size="md">
                ...
                <FormGrid>
                    <Field label="Warehouse" required><Select value={form.warehouseID.toString()} onChange={(e) => set("warehouseID", parseInt(e.target.value) || 0)}><option value="0">Select...</option>{warehouses.map(w => <option key={w.warehouseID} value={w.warehouseID}>{w.warehouseName}</option>)}</Select></Field>
                    <Field label="Product" required><Select value={form.productID.toString()} onChange={(e) => set("productID", parseInt(e.target.value) || 0)}><option value="0">Select...</option>{products.map(p => <option key={p.productID} value={p.productID}>{p.productName}</option>)}</Select></Field>
